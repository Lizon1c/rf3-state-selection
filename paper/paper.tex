...
\author{Author One$^{1\ast}$, Author Two$^{1}$, Author Three$^{2}$\\
\small $^{1}$Affiliation One; $^{2}$Affiliation Two\\
\small $^{\ast}$Corresponding author: author@example.com}
\date{}
...
publicly available \cite{corley2025}. Analysis code and result files are
available from the authors (repository link to be added upon publication).
